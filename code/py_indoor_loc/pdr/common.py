...
def compute_step_heading(step_timestamps: np.ndarray, headings: np.ndarray) -> np.ndarray:
...
def compute_rel_positions(stride_lengths: np.ndarray, step_headings: np.ndarray) -> np.ndarray:
...
class RelPositionPredictor(object):
  ...
  def predict(self, path_data_collection) -> np.ndarray:
    ...
    if self._use_ahrs:
      _, heading = compute_earth_acce_heading_ahrs(path_data_collection.acce, path_data_collection.ahrs)
    else:
...
def compute_cumulative_step_positions(rel_step_positions: np.ndarray) -> np.ndarray:
