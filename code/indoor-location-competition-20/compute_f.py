...
def correct_trajectory(original_xys, end_xy):
  ...
  for i in np.arange(1, np.size(original_xys, 0)):
    angle_CpAX = np.arctan2(original_xys[i, 1] - A[1],
                            original_xys[i, 0] - A[0])
...
def correct_positions(rel_positions, reference_positions):
  ...
  for i, rel_ps in enumerate(rel_positions_list):
    ...
    if i == 0:
      ...
      corrected_positions = np.append(corrected_positions,
                                      corrected_ps[1:],
                                      axis=0)
...
def init_parameters_filter(sample_freq, warmup_data, cut_off_freq=2):
  order = 4
  filter_b, filter_a = signal.butter(order, cut_off_freq / (sample_freq / 2),
                                     'low', False)
...
def compute_steps(acce_datas):
  ...
  for i in np.arange(0, np.size(acce_datas, 0)):
    ...
    acce_mag_filt, filter_zf = signal.lfilter(filter_b,
                                              filter_a, [acce_mag],
                                              zi=filter_zf)
    ...
    if (acce_binarys[-1] == 0) and (acce_binarys[-2] == 1):
      if state_flag == 0:
        ...
        state_flag = 1
      elif (state_flag == 1) and ((acce_data[0] - acce_max[0])
                                  <= interval_threshold) and (acce_mag_filt
                                                              > acce_max[1]):
        acce_max[:] = acce_data[0], acce_mag_filt
      elif (state_flag == 2) and ((acce_data[0] - acce_max[0])
                                  > interval_threshold):
        acce_max[:] = acce_data[0], acce_mag_filt
        state_flag = 1
    ...
    if step_criterion == 2:
      ...
      if (acce_binarys[-1] == -1) and (acce_binarys[-2] == 0) and (np.sum(
          acce_binarys[:-2]) > 1):
        step_flag = True
    else:
      if (acce_binarys[-1] == 0) and acce_binarys[-2] == -1:
        if (state_flag == 1) and ((acce_data[0] - acce_min[0])
                                  > interval_threshold):
          acce_min[:] = acce_data[0], acce_mag_filt
          state_flag = 2
          step_flag = True
        elif (state_flag == 2) and ((acce_data[0] - acce_min[0])
                                    <= interval_threshold) and (acce_mag_filt
                                                                < acce_min[1]):
          acce_min[:] = acce_data[0], acce_mag_filt
    if step_flag:
...
def compute_stride_length(step_acce_max_mins):
  ...
  for i in range(0, step_timeperiod.shape[0]):
    step_timeperiod_data = (step_acce_max_mins[i + 1, 0] -
                            step_acce_max_mins[i, 0]) / 1000
    step_timeperiod_temp = np.append(step_timeperiod_temp,
